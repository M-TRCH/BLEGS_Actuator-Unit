...
# Maximum reach ≈ 250mm (L_AC + L_CE), use 65% for safe workspace
DEFAULT_STANCE_HEIGHT = -160.0  # mm (was -200, reduced to stay in safe workspace)
...
# Set to True to skip moving to IK-calculated home position
# This prevents motor cutoff when motor init angle doesn't match IK solution
SKIP_HOME_TRANSITION = True  # Skip home movement - start gait from current position
HOME_TRANSITION_DURATION = 5.0  # Longer duration for safety (seconds)

# --- Single Motor Mode ---
SINGLE_MOTOR_MODE = False  # Set to True for single motor testing
SINGLE_MOTOR_OSCILLATION = 5.0   # degrees (reduced from 10 for safety)
SINGLE_MOTOR_PERIOD = 4.0  # seconds (increased from 2 for slower movement)
...
leg_states = {
    'FR': {'target_angles': [MOTOR_INIT_ANGLE_RAD, MOTOR_INIT_ANGLE_RAD], 'actual_angles': [MOTOR_INIT_ANGLE_RAD, MOTOR_INIT_ANGLE_RAD], 'target_pos': [0.0, -160.0], 'phase': 0, 'color': 'red'},
    'FL': {'target_angles': [MOTOR_INIT_ANGLE_RAD, MOTOR_INIT_ANGLE_RAD], 'actual_angles': [MOTOR_INIT_ANGLE_RAD, MOTOR_INIT_ANGLE_RAD], 'target_pos': [0.0, -160.0], 'phase': 0, 'color': 'blue'},
    'RR': {'target_angles': [MOTOR_INIT_ANGLE_RAD, MOTOR_INIT_ANGLE_RAD], 'actual_angles': [MOTOR_INIT_ANGLE_RAD, MOTOR_INIT_ANGLE_RAD], 'target_pos': [0.0, -160.0], 'phase': 0, 'color': 'orange'},
    'RL': {'target_angles': [MOTOR_INIT_ANGLE_RAD, MOTOR_INIT_ANGLE_RAD], 'actual_angles': [MOTOR_INIT_ANGLE_RAD, MOTOR_INIT_ANGLE_RAD], 'target_pos': [0.0, -160.0], 'phase': 0, 'color': 'green'}
}
...
def main():
    ...
    if SKIP_HOME_TRANSITION:
        ...
        print(f"    Set SKIP_HOME_TRANSITION = False to enable home movement")
    else:
        smooth_move_to_home_position(leg_motors, home_angles_all, duration_s=HOME_TRANSITION_DURATION)
